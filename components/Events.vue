<template>
  <div class="p-4 md:p-8">
    <h2 class="font-danaBold text-4xl leading-normal dark:text-white">
      رویدادها
    </h2>
    <div
      class="flex flex-wrap items-center mt-8 w-full border-b dark:border-gray-700 font-danaReg py-4"
    >
      <div
  ...
  <div class="p-4 pt-0 md:p-8 md:pt-0 grid grid-cols-3 gap-8">
    <template v-for="n in 12" :key="n">
      <div v-if="eventsLoading" class="card col-span-3 lg:col-span-1">
        <Skeleton
      ...
      >
        <div class="absolute w-full top-0 right-0 p-4 flex z-50 font-danaReg">
          <div
            v-if="!item.type"
            class="rounded-full bg-white dark:bg-gray-700 text-prblue dark:text-french px-4 py-2 flex items-center font-bold"
          >
            <i class="pi pi-desktop me-2" />
            آنلاین
          </div>
          <div
            v-else
          ...
          <Button
            class="rounded-full px-4 py-2 flex items-center font-bold ms-2"
            severity="info"
          >
            {{ item.category }}
          </Button>
        ...
        <div class="w-1/2 lg:w-full">
          <div class="w-full p-4 my-4">
            <div class="flex items-center justify-start">
              <span>
                <i class="pi pi-map-marker text-[1rem] text-prblue" />
              </span>
              <span class="ms-2 text-space dark:text-gray-300">{{
                item.location
              }}</span>
            </div>
          </div>
          <div class="w-full p-4 flex-wrap">
            <div
              class="w-full font-bold text-xl group-hover:text-prblue duration-300 dark:text-french"
            >
              {{ item.title }}
            </div>
            <div class="text-space dark:text-gray-400 mt-4">
              {{ item.description }}
            </div>
          </div>
        </div>
